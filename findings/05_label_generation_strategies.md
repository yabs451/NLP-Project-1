...
**Each dataset was written by the previous generation**: the row "dataset for
gen 4" was generated by the generation-3 model and used to train generation 4.
Counts, with rates in parentheses:

| dataset for gen (written by gen − 1) | argmax | T = 1 | T = 3 | T = 5 |
| --- | ---: | ---: | ---: | ---: |
...
**The endpoint is close to a uniform draw over the five labels.** At temperature
5 generation 4 the three columns are 0.2009 / 0.2016 / 0.5975, near the
0.20 / 0.20 / 0.60 that picking one of the five labels at random would give,
since one is correct, one is the other context label and three are outside. The
generated marginal is similarly close to even:
201,585 / 199,069 / 198,829 / 199,779 / 200,738 across the five labels, against
true counts of 188,451 / 187,307 / 187,256 / 187,105 / 249,881. So the parent did
not acquire a pronounced preference for a particular label, and the true
distribution's over-representation of label 4 is no longer visible in what it
generated. Temperature 3 generation 4 is close behind
(201,111 / 195,817 / 193,546 / 204,878 / 204,648).

These are **empirical frequencies of sampled tokens**, counted three ways: the
label marginal, the correct-versus-generated table, and the correct/other/outside
split. None of them recovers the model's probability vector on any individual
question — many different per-question distributions produce the same marginal —
and we did not measure those probabilities.
...
corrupted dataset. Choosing uniformly among the four wrong labels would give
0.75, so these counts are **consistent with** that baseline. They do not on their
own establish that all four wrong labels were equally likely, nor do they rule
out some degree of confusion between the two context labels: other error
distributions also produce a three-quarters share. What can be said is that the
errors are not *concentrated* on the other context label, which is what a
confusion account would predict. The argmax
...
**Argmax and temperature-1 sampling remained broadly stable over these four
...
Clearest in the **query-label loss**, which shows a substantial rise at
generations where accuracy is still high: at temperature 3 it rises
0.011 → 0.076 → 0.696 while accuracy is still 0.986, and at temperature 5 it
reaches 0.262 at generation 1 while accuracy is still 0.994. Generations are the
only time resolution we have across a chain, so this says the two measures were
out of step at those generations — not that we established when either began to
move, nor that loss generally leads accuracy.
...
In the collapsed models every ablation effect is within ±0.3 points. Intact
accuracy in those models is already near the chance baseline, which makes the
comparison hard to read: a small effect there is not evidence that the head was
unimportant, but neither is "nothing left to remove" a sufficient explanation —
chance is a reference level, not a floor a model cannot fall below, and these
...
temperature-1 sampling corrupted at most 0.049% of targets — that is the
following-label figure, 489 per million at temperature 1 generation 4; the
query-label maximum is lower at 401 — and both chains stayed broadly stable over
four generations. Temperature 3 and temperature 5 corrupted 5%
...
outputs; on the measures we took, the symbol side of the generated data stayed
close to the correct data, with no pronounced concentration. That is a negative
result and is reported as one — "close on these measures", not "identical".

It also fits the model measurements: the trained children keep symbol loss close
to ln 2 in every condition while their label accuracy falls, so the data and the
models agree about which output deteriorated. The small temperature-3
...
collapsed runs the induction score was **below 0.05 at every one of the 12
analysed checkpoints** and accuracy never exceeded 0.543 at any of 201 logged
points. That is what was recorded. The stronger statement — that the pattern
never formed at all — goes beyond it: 43 checkpoints per run were never measured.
And an attention score measures where heads attend, not what a circuit
contributes to the output, so "the induction circuit stopped functioning" is an
interpretation of these measurements rather than something they establish.
...
ordering remains unresolved. The one partial dissociation is that substantial query *loss* increases appear at
generations where accuracy is still high — temperature 5 generation 1 has loss 23×
