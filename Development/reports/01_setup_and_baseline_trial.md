...
not guarantee disjoint examples in a finite synthetic task. *(Correction: this
originally added "there are only 78,400 possible training sequences under these
settings". 78,400 is the number of distinct questions counted in one generated
dataset, not the size of the question space, which we never established. The
point about disjointness does not depend on it.)*
