...
| `scripts/verify_trial.py` | Reloads checkpoints and checks that training and saving worked | `load_checkpoint()`, `main()` | A run folder (`config.json`, `log.h5`, `eval_data.h5`, `checkpoints/`) | `verification.json` | After a run that saved evaluator data |
...
8. **`fsl_train` is retained** and is labelled *training-distribution
   evaluation*. It draws fresh sequences from the training distribution, so it is
   not guaranteed disjoint from training and is a diagnostic, not validation.
   *(Correction: this originally justified that with "only 78,400 possible
   sequences under these settings". That figure is the number of distinct
   questions counted in one generated dataset, not the size of the question
   space, which we never established. The conclusion stands without it.)*
...
10. **Training is unaffected** — checked below, by comparing the runs described
    there rather than by proof.
